...
func main() {
	...
	// Initialize logger
	logger := utils.NewLogger("admin-service", cfg.LogLevel, cfg.LogFormat)
	logger.Info("Starting Admin Service", "version", "1.0.0")
	...
	go func() {
		...
		logger.Info("Admin Service started", "address", lis.Addr().String())
